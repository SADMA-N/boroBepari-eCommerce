  ...
  const paymentOptions = [
    {
      ...
      details: (
        <div className="mt-3 text-sm text-gray-600 bg-white p-3 rounded border border-gray-100">
          ...
          <p className="text-xs text-gray-500">Secure payment via bKash or Credit/Debit Card</p>
      ...
      details: (
        <div className="mt-3 text-sm text-gray-600 bg-white p-3 rounded border border-gray-100">
          <div className="flex justify-between mb-1">
            ...
            <span className="font-bold text-orange-600">{formatCurrency(depositAmount)}</span>
          </div>
          ...
          <section>
            ...
            <div className="space-y-3">
              {paymentOptions.map((option) => (
                <div 
                  key={option.id}
                  onClick={() => !option.disabled && handleMethodSelect(option.id)}
                  className={`
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <option.icon size={20} className={selectedMethod === option.id ? 'text-orange-600' : 'text-gray-500'} />
             ...
             {selectedMethod && (
               <div className="py-4 space-y-2 bg-orange-50 -mx-6 px-6 border-b border-orange-100">
                 <div className="flex justify-between text-sm font-medium text-gray-800">
                   <span>Due Now</span>
                   <span className="text-orange-700">{formatCurrency(dueNow)}</span>
                 </div>
                 {dueLater > 0 && (
