    ...
    <div className="max-w-[1440px] mx-auto px-6 py-8">
      <div className="flex flex-col md:flex-row justify-between md:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage incoming quote requests from buyers</p>
        </div>
      </div>
...
function SupplierRFQCard({ rfq, onSendQuote, isActionable }: { rfq: MockRfq, onSendQuote: () => void, isActionable: boolean }) {
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="bg-blue-50 text-blue-700 text-xs font-mono px-2 py-0.5 rounded">{rfq.rfqNumber}</span>
            <span className="text-gray-400 text-xs">•</span>
            <span className="text-sm text-gray-500">{format(rfq.createdAt!, 'PP')}</span>
          </div>
          <h3 className="text-lg font-bold text-gray-900 mb-1">{rfq.product.name}</h3>
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-600 mt-2">
             <div className="flex items-center gap-2">
               <span className="font-semibold text-gray-900">{rfq.quantity} {rfq.product.unit}s</span>
               <span className="bg-gray-100 px-1.5 rounded text-xs">Qty</span>
             </div>
             <div className="flex items-center gap-2">
               <span className="font-semibold text-blue-600">{formatBDT(Number(rfq.targetPrice))}</span>
               <span className="bg-gray-100 px-1.5 rounded text-xs">Target</span>
             </div>
             <div className="flex items-center gap-1.5">
               <MapPin size={14} className="text-gray-400" />
               <span className="truncate max-w-[150px]" title={rfq.deliveryLocation}>{rfq.deliveryLocation}</span>
             </div>
          </div>
        </div>
...
function QuoteResponseModal({ isOpen, onClose, rfq, onSuccess }: { isOpen: boolean, onClose: () => void, rfq: MockRfq, onSuccess: () => void }) {
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
           ...
           <div className="bg-blue-50 p-4 rounded-lg mb-6 border border-blue-100">
             <div className="flex justify-between items-start mb-2">
               ...
               <span className="text-xs bg-white text-blue-700 px-2 py-1 rounded font-mono font-bold">Qty: {rfq.quantity}</span>
             </div>
             <div className="text-sm text-blue-800 flex gap-4">
               <span>Buyer Target: <span className="font-bold">{formatBDT(Number(rfq.targetPrice))}</span></span>
             </div>
           </div>
           ...
           <form onSubmit={handleSubmit} className="space-y-6">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <div>
                 <label className="block text-sm font-medium text-gray-700 mb-1">Unit Price (BDT) <span className="text-red-500">*</span></label>
                 ...
                 {Number(unitPrice) > Number(rfq.targetPrice) && unitPrice !== '' && (
                    <p className="text-xs text-orange-600 mt-1 flex items-center gap-1">
                      <AlertCircle size={12} />
                      Higher than target price ({formatBDT(Number(rfq.targetPrice))})
                    </p>
                 )}
               </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <div>
                 <label className="block text-sm font-medium text-gray-700 mb-1">Est. Delivery Time</label>
                 <input 
                    type="text"
                    value={deliveryTime}
