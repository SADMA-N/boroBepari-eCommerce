...
function NotificationsPage() {
  ...
  const filteredNotifications = useMemo(() => {
    ...
    switch (filter) {
      ...
      case 'system':
        return byOrder(
          notifications.filter((n) => n.category === filter && !n.archived),
        )
      case 'archived':
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-white dark:bg-slate-900 rounded-xl border dark:border-slate-800 p-4 flex flex-wrap gap-3 items-center">
              ...
              <input
                ...
                className="ml-auto text-xs border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-900 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-400"
              />
            </div>

            {filteredNotifications.length === 0 ? (
              <div className="bg-white dark:bg-slate-900 rounded-xl border dark:border-slate-800 p-10 text-center text-gray-500 dark:text-gray-400">
                <Bell
                  size={36}
                  ...
                  <div key={notif.id} className="p-4 flex gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p
                            className={`text-sm ${notif.isRead ? 'font-medium text-gray-800 dark:text-gray-200' : 'font-bold text-gray-900 dark:text-white'}`}
                          >
                            {notif.title}
                          </p>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border dark:border-slate-800 p-5">
              ...
              <div className="space-y-3 text-sm">
                <ToggleRow
                  ...
                  onToggle={(value) =>
                    updatePreferences({ orderPlaced: value })
                  }
                />
                <ToggleRow
                  ...
                  onToggle={(value) =>
                    updatePreferences({ orderConfirmed: value })
                  }
                />
                <ToggleRow
                  label="Order shipped"
                  ...
                  onToggle={(value) =>
                    updatePreferences({ orderShipped: value })
                  }
                />
                <ToggleRow
                  ...
                  onToggle={(value) =>
                    updatePreferences({ orderOutForDelivery: value })
                  }
                />
                <ToggleRow
                  label="Delivered"
                  ...
                  onToggle={(value) =>
                    updatePreferences({ orderDelivered: value })
                  }
                />
                <ToggleRow
                  ...
                  onToggle={(value) =>
                    updatePreferences({ orderCancelled: value })
                  }
                />
                <ToggleRow
                  ...
                  onToggle={(value) =>
                    updatePreferences({ refundProcessed: value })
                  }
                />
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border dark:border-slate-800 p-5">
              ...
              <div className="space-y-3 text-sm">
                ...
                <ToggleRow
                  ...
                  onToggle={(value) =>
                    updatePreferences({
                      channels: { ...preferences.channels, email: value },
                    })
                  }
                />
                <ToggleRow
                  ...
                  onToggle={(value) =>
                    updatePreferences({
                      channels: { ...preferences.channels, sms: value },
                    })
                  }
                />
                <ToggleRow
              ...
              {isLoadingAlerts ? (
                ...
                <div className="space-y-3">
                  {stockAlerts.map((alert) => (
                    <div
                      key={alert.id}
                      className="border border-gray-100 dark:border-slate-800 rounded-lg p-3 text-sm"
                    >
...
function ToggleRow({
  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-between text-sm text-gray-700 dark:text-gray-300 transition-colors"
    >
      <span>{label}</span>
      <span
        className={`w-10 h-5 rounded-full flex items-center px-1 transition-colors ${value ? 'bg-green-500' : 'bg-gray-200 dark:bg-slate-700'}`}
      >
        <span
          className={`w-4 h-4 rounded-full bg-white dark:bg-gray-100 shadow transform transition-transform ${value ? 'translate-x-5' : ''}`}
        />
      </span>
    </button>
  )
