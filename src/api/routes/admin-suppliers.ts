...
const router = new Hono<AdminEnv>().basePath('/admin/suppliers')
...
router.get('/', async (c) => {
  ...
  // For each seller, fetch product count and GMV
    ...
    allSellers.map(async ({ seller, supplier }) => {
      ...
      return {
        ...
        bank: {
          ...
          accountNumberMasked: seller.accountNumber
